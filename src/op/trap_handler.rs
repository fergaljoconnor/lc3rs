...
pub(crate) fn getchar<IO: IOHandle>(vm: &mut VM<IO>) {
    let ch = vm.getchar() as u16;
    vm.reg_write(RR0, ch);
}

pub(crate) fn trap_out<IO: IOHandle>(vm: &mut VM<IO>) {
    let ch = vm.reg_read(RR0) as u8 as char;
    vm.putchar(ch);
}

pub(crate) fn put_string<IO: IOHandle>(vm: &mut VM<IO>) {
...
pub(crate) fn trap_in<IO: IOHandle>(vm: &mut VM<IO>) {
...
pub(crate) fn put_byte_string<IO: IOHandle>(vm: &mut VM<IO>) {
    let mut next_address = vm.reg_read(RR0);
...
pub(crate) fn trap_halt<IO: IOHandle>(vm: &mut VM<IO>) {
